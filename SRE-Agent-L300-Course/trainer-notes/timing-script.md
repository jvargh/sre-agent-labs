...
## Day 1 — Labs 1–7 (8.0 hours)

### 09:00–09:45 — Lab 1: Promotion Playbook (45 min)
...
| 9:40–9:45 | Wrap: Lab 1 rule recap. "Never promote to Autonomous on SendOutlookEmail + az write without Lab 9 hooks in place." | Poll: "Who has a use case for Autonomous + write?" → good sign if 30 % yes. |
...
**SYNC PULSE:** Lab 1 ends at 9:45. All three tracks (PagerDuty, ServiceNow, Azure Monitor) start Lab 2 at same time.

---

### 09:45–10:45 — Lab 2: Incident Platform Connection (60 min)
...
| 10:40–10:45 | Wrap: Why delete quickstart? "L300 bug: double-route. Now you decide the routing logic in Lab 3." (5 min) | All three | All three | All three |
...
**SYNC PULSE:** Lab 2 ends at 10:45. All tracks reconverge for break.
...
### 11:00–12:30 — Lab 3: Response Plans + Deep Investigation Mode 2 (90 min)
...
| 12:20–12:30 | Wrap: Failure mode talk-track (10 min). Show unified grid; point out where overlapping plans would be visible. Segue to Lab 4. | All 13 attendees show green checkmarks on at least one response plan. |

**Contingency:**
- A custom agent can't be created? → Use trainer's pre-baked agent; catch up in async later.
- Test incident not routing? → Check connector status (Lab 2 checkpoint); reset Builder session.
...
**SYNC PULSE:** Lab 3 ends at 12:30. All tracks break for lunch.
...
### 13:00–14:15 — Lab 4: Skills Authoring (75 min)
...
| 14:00–14:15 | Stretch + wrap (15 min): Discuss llowed_skills syntax for custom agents (Lab 6 preview). Show VS Code extension loop. Anti-patterns: description without trigger phrases → never loads. | Trainer demos the VS Code extension (no editing required; just demo). |
...
### 14:30–15:30 — Lab 5: Custom Tools I — Kusto + Link (60 min)
...
| 14:40–14:55 | Step 2 (15 min): Author Kusto tool. Builder → Agent Canvas → Create Tool → Kusto tool. Demo: bounded time window ##timeRange##, bounded result 	ake 100, explicit project. Attach to p1-investigator from Lab 3. | Each attendee has one Kusto tool with at least 2 parameters. |
...
### 15:30–17:00 — Lab 6: Custom Agents in YAML (90 min)
...
| 16:35–16:50 | Step 3 (15 min): Wire the chain. Update Response Plan B (Lab 3) to dispatch to incident_triager. Canvas view shows handoff edges. | Response Plan B now dispatches to incident_triager; chain visible in canvas. |
| 16:50–17:00 | Wrap (10 min): Context sharing recap. Tools per agent budget. Segue to Lab 7 MCP integrations. | All attendees have three specialists + notifier wired. |
...
### 17:00–18:15 — Lab 7: MCP Integrations II (75 min)
...
## Day 2 — Labs 8–13 (9.0 hours)

### 09:00–10:30 — Lab 7 recap + Lab 8: Custom Tools II — Python Tools (90 min)

| Time | Activity | Checkpoint |
|------|----------|-----------|
| 09:00–09:10 | Day 2 welcome + Lab 7 recap (10 min). "Yesterday you wired three MCP connectors. Today: Python tools with managed-identity scoping." | Attendees logged in, Day 2 slide visible. |
...
| 10:25–10:30 | Wrap (5 min): Attach Tool 3 to db-expert (Lab 6). Talk-track: Python tool vs MCP connector decision table. | Tool 3 attached to db-expert. |
...
### 10:45–12:15 — Lab 9: Agent Hooks (90 min)
...
| 12:05–12:15 | Wire + wrap (10 min): All three hooks attached at **agent level** (cover every custom agent). Re-fire Lab 3 P1 incident; verify audit messages appear. Test synthetic m -rf / block. | Audit messages visible in the incident thread. Dangerous command blocked. |
...
**SYNC PULSE:** Lab 9 ends at 12:15. Core L300 content done. All three tracks on track. Safety culture reinforced: "Before Autonomous, hooks are in place."
...
### 13:00–14:15 — Lab 10: Audit, FinOps & Observability (75 min)
...
| 14:10–14:15 | Wrap (5 min): Trace correlation. Segue to Lab 11. | Attendees have a saved workbook + understand trace flow. |
...
### 14:30–16:00 — Lab 11: Enterprise Topology (90 min)
...
**Contingency:**
- Entra admin unavailable? → Immediately shift to **Fallback: Lecture-only (30 min) + Q&A (60 min)**. Update feedback survey: "Plan to attempt Lab 11 lab post-workshop? Yes/No." Offer async post-workshop with attendee's own Entra admin.
...
### 16:15–17:45 — Lab 12: Configuration as Code (90 min)
...
| 16:30–17:00 | Part 1 (30 min): Attendees choose Bicep OR Terraform. Reference repo provided. Deploy agent resource + UAMI + role assignments (per Lab 1 matrix) + App Insights/Log Analytics. Param the model provider (Anthropic vs Azure OpenAI). | Every attendee has agent-iac.bicep (or .tf) file with at least one parameter. |
| 17:00–17:20 | Part 2 (20 min): REST API v2. Attendees use provided PowerShell/Python client to PUT /api/v2/extendedAgent/agents/{agentName} with the YAML schema from Lab 9 (hooks here, not portal). | Every attendee has tested the REST client in a dry-run (no deployment yet). |
...
| 17:30–17:45 | Part 4 + wrap (15 min): Drift control query (Azure Activity Log) to flag out-of-band portal changes. Segue to Lab 13 capstone. | Attendees understand: "Configuration = Code; enforce via CI and drift detection." |

**Contingency:** If Bicep/Terraform deployment fails, attendees reference the pre-built template; continue with REST API v2.

---

### 17:45–18:00 — Lab 13 Setup (15 min)
...
| Time | Activity | Checkpoint |
|------|----------|-----------|
| 17:45–17:55 | Setup (10 min): Trainer displays the three synthetic incidents queued: [TEST] P3 high latency, [TEST] P1 db corruption, [TEST] P2 api 500s. Walk through expected behavior per Lab 13 guide. | All attendees see the incident list. No surprises. |
| 17:55–18:00 | Q&A (5 min): Any questions on routing, hooks, or the capstone scoring rubric? | No critical blockers. |

---

### 18:00–19:30 — Lab 13: Production Rollout Playbook + Capstone Drill (90 min)

| Time | Activity | Checkpoint |
|------|----------|-----------|
| 18:00–18:10 | Rollout playbook review (10 min). Read aloud the 10 steps (Lab 13 §MD): Land PR → delete quickstart → author skills → wire agents → add tools → wire hooks → build workbook → set tiers → rollout shadow mode → operations cadence. Attendees have a one-page printed/digital copy. | All attendees have the 1-pager. |
...
| 19:15–19:30 | Wrap + debrief (15 min). Review capstone scoring. Congratulations. Post-workshop next steps: (1) land agent-as-code PR within 2 weeks, (2) feedback survey, (3) optional async Lab 11 if needed. | Attendees receive capstone scores. Positive tone. |
...
1. **After each session:** Verify at least 80 % of attendees have the checkpoint green (demo or screenshot).
...
3. **Synchronization pulse points (Lab 2, Lab 3, Lab 13):** All three tracks must restart at the same time. Waiting 15 min for a slow track to finish is acceptable; waiting 45 min is not (break early, catch up async).
...
- **"Stretch task"** → optional if time permits; not required for later sessions.
- **"Lab 9 first, then Autonomous"** → production safety culture. Never demonstrate Autonomous without hooks. Never.
...
**Note:** If you only have one day, cut Lab 5, Lab 7, or Lab 11; do NOT cut Lab 1, Lab 3, Lab 9, Lab 10, or Lab 13. Prioritize the promotion playbook, response plans, hooks, and capstone.
