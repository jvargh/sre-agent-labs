...
# M1 — Promotion Playbook: Privileged + Autonomous Safely

> **Format:** Lecture + paired exercise (45 min) — no hands-on lab.
> **Slides:** M1 deck (printable handout included).
...
> This is the matrix attendees leave with. Print it as a handout from the M1 slide deck.
...
| Run mode | Review | Autonomous | Autonomous | Review → Autonomous after bake-in | Review | Autonomous w/ Hooks (M9) |
...
> **Run Modes do *not* gate non-Azure tool calls** (email, Teams, MCP). **Hooks do** (M9).
...
This distinction is the single most common production mistake at L300. Hooks (covered in [M9](../module-M9-agent-hooks/README.md)) are the enforcement boundary for non-Azure actions.
...
Each attendee has a completed matrix they will reference in every subsequent module. In M3, you will wire response plans that match these decisions. In M9, you will implement the hooks that make Autonomous safe.
