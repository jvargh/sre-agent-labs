...
# M6 — Custom Agents in YAML: Handoff Chains
...
> **Troubleshooting:** If YAML view is empty, the custom agent may not have been saved in M3. Re-create and save.
...
tools:
  - GetRecentDbErrors        # Kusto tool from M5
...
tools:
  - GetRecentErrors          # Kusto tool from M5
  - RunAzCliReadCommands
```
...
> ⚠️ **Production safety:** The `notifier` is the **only** custom agent in this chain running Autonomous with `SendOutlookEmail` / `SendTeamsMessage` tools. Per M1, this requires a PostToolUse hook (installed in M9).
...
1. Navigate to **Builder → Incident Response Plans → Plan B** (P1/P2 plan from M3).
