...
|---------|-------------|------------|
| YAML view empty for `p1-investigator` | Agent not saved properly in M3 | Re-create and save `p1-investigator` in Agent Canvas |
...
| `notifier` runs `az cli` commands despite system prompt | System prompt is advisory, not a hard gate | Add a PostToolUse hook (M9) to block non-notification tools on `notifier` |
