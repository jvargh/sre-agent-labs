...
3. Tools: `RunAzCliReadCommands`, Log Analytics read-only.
   - *Note:* We will attach the Kusto tool (Lab 5) and Python tool (Lab 8) in later Labs. Leave hook placeholders for Lab 9.
4. Click **Save**.

> **Expected state:** `p1-investigator` appears in the Agent Canvas.
> **Troubleshooting:** If Autonomous mode is greyed out, confirm your agent resource has Privileged permissions set (Lab 1 matrix).
...
> **Troubleshooting:** If no incidents match, fire a `[TEST] P3` synthetic incident first (see Lab 2).
...
> **Troubleshooting:** If the plan doesn't save, check for overlapping filters with any surviving quickstart plan (should have been deleted in Lab 2).
...
A Privileged/Autonomous P1 plan **without** a PostToolUse hook means the agent can execute dangerous commands unchecked. This is the core Lab 9 conversation — hooks are the enforcement boundary.

> **Forecast:** In Lab 9, you will add Stop + PostToolUse hooks that make Response Plan B production-safe.
...
- [ ] Attendee understands the missing-hook risk (to be addressed in Lab 9).
