...
# M10 — Audit, FinOps & Observability: KQL on customEvents, Token-Cost Analytics, Model-Tier Strategy
...
| Time | Checkpoint |
|------|-----------|
| 0:00 | Lab start — verify M9 hooks active |
...
- **Troubleshooting:** If few events appear, re-fire a test incident from M3 and wait 2–3 minutes for telemetry to flush.
...
- **Action:** In the KQL editor, author:
  ...
  ```kql
  customEvents
  | where customDimensions.ThreadId == "<paste-a-thread-id-from-M3>"
...
- **Troubleshooting:** If no results, verify the `ThreadId` value. Copy it from a conversation URL or the M3 incident thread.
...
- **Troubleshooting:** If `IncidentActivitySnapshot` events are missing, ensure the M3 response plans have processed at least one incident.
...
- **Action:** Pick a `TraceId` from a M3 incident thread. Run:
...
- **Expected state:** Shows any out-of-band changes made to the agent via the portal (config drift detection — pairs with M12).
...
Proceed to [M11 — Enterprise Topology](../module-M11-enterprise-topology/README.md).
